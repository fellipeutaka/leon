...
A **flow** is a path through the skills in this repo. Most paths run along one **main flow**, and two **on-ramps** merge onto it. Everything else is standalone, or a vocabulary layer that runs underneath. Delivery tooling, including stacked PRs, sits underneath the main flow rather than replacing it.
...
Keep steps 1–3 in **one unbroken context window** — don't compact or clear until after `/to-tickets` — so the grilling, spec, and tickets all build on the same thinking. Each `/implement` then starts fresh, working from the ticket and the durable repository/issue state.
